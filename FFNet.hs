...
--Given net and data, does gradient descent
...
--Generates predictions based on data generator and net
generatePredictions :: Int -> Net -> Pairlist -> Double -> Int -> Int -> IO ([Double])
generatePredictions genr8r net samps rate gens num = do
  ...
  putStrLn $ show outs
  return (zipWith (\x y -> quadCost outs x y) (map fst samps) (map snd samps))  
